//! Source I/O marks → virtual shot → Story part (tonovi / offovi).
...
impl SourceMarks {
    pub fn set_in(&mut self, clip_id: &str, sec: f64) {
        self.clip_id = Some(clip_id.to_string());
        self.mark_in = Some(sec.max(0.0));
        if self.mark_out.is_some_and(|o| o <= sec) {
            self.mark_out = None;
        }
    }

    pub fn set_out(&mut self, clip_id: &str, sec: f64) {
        let sec = sec.max(0.0);
        if self.clip_id.as_deref() != Some(clip_id) {
            self.clip_id = Some(clip_id.to_string());
            self.mark_in = None;
        }
        self.mark_out = Some(sec);
    }

    pub fn summary(&self) -> String {
        match (self.clip_id.as_deref(), self.mark_in, self.mark_out) {
            (Some(c), Some(i), Some(o)) => format!("IN={i:.2} OUT={o:.2} clip={c}"),
            (Some(c), Some(i), None) => format!("IN={i:.2} OUT=? clip={c}"),
            (Some(c), None, Some(o)) => format!("IN=? OUT={o:.2} clip={c}"),
...
pub fn create_segment(
        ...
        (marks.clip_id.as_deref(), marks.mark_in, marks.mark_out)
    {
        ...
        let resp = host.create_virtual_shot(project_id, clip_id, inn, out)?;
        let shot_id = resp
            .get("shot")
            .and_then(|s| s.get("shot_id"))
            .and_then(Value::as_str)
            .ok_or_else(|| "virtual-shot odgovor bez shot_id".to_string())?
            .to_string();
        let _ = host.shot_select(project_id, &shot_id)?;
        let snap = host.create_part(project_id, kind, Some(&shot_id))?;
        let part_id = snap
            .get("selected_part_id")
            .and_then(Value::as_str)
            .unwrap_or("")
            .to_string();
        return Ok((format!("{kind} iz IN/OUT → shot {shot_id}"), part_id));
    }
...
/// Absolute source seconds for a part at wrap playhead (from story_state parts).
pub fn source_sec_at_part(state: &Value, part_id: &str, local_sec: f64) -> Option<(String, f64)> {
    let part = state
        .get("parts")
        .and_then(Value::as_array)?
        .iter()
    ...
    let in_sec = part
        ...
        .unwrap_or(0.0);
    Some((clip_id, (in_sec + local_sec.max(0.0)).max(0.0)))
}
...
pub fn selected_slot_duration(state: &Value) -> Option<(String, f64)> {
    let selected = state
    ...
    let dur = slot
        ...
        .filter(|d| *d > 0.02)?;
    Some((selected.to_string(), dur))
}

pub fn slot_duration_by_id(state: &Value, slot_id: &str) -> Option<f64> {
    let slot = state
        .get("marker_slots")
        .and_then(Value::as_array)?
        .iter()
        .find(|s| s.get("slot_id").and_then(Value::as_str) == Some(slot_id))?;
    slot.get("duration_sec")
        .and_then(Value::as_f64)
        .filter(|d| *d > 0.02)
        .or_else(|| {
            let a = slot.get("start_sec").and_then(Value::as_f64)?;
...
/// Apply IN at `sec`, OUT = IN + selected slot duration (from SQLite selection).
pub fn apply_mark_in_fit_duration(
    marks: &mut SourceMarks,
    clip_id: &str,
    in_sec: f64,
    state: &Value,
) -> Result<(f64, f64), String> {
    let (_slot_id, dur) = selected_slot_duration(state)
        .ok_or_else(|| "Nema odabranog slota / trajanja (fokus prazni slot)".to_string())?;
    let inn = in_sec.max(0.0);
    let out = inn + dur;
...
pub fn create_cover_from_marks(
    ...
    let (clip_id, inn, out) = match (marks.clip_id.as_deref(), marks.mark_in, marks.mark_out) {
        (Some(c), Some(i), Some(o)) if o > i + 0.02 => (c, i, o),
    ...
    let cover_id = snap
        ...
        .to_string();
    Ok((
        format!("cover na {slot_id} ← shot {shot_id} ({inn:.2}–{out:.2}s)"),
        cover_id,
    ))
}
...
pub fn cover_confirm_summary(marks: &SourceMarks, state: &Value, slot_id: &str) -> String {
    let dur = slot_duration_by_id(state, slot_id).unwrap_or(0.0);
    let inn = marks.mark_in.unwrap_or(0.0);
    let out = marks.mark_out.unwrap_or(0.0);
    let clip = marks.clip_id.as_deref().unwrap_or("?");
    format!(
        "POTVRDI cover · slot={slot_id} · dur={dur:.3}s · IN={inn:.3} OUT={out:.3} · clip={clip}"
    )
}
